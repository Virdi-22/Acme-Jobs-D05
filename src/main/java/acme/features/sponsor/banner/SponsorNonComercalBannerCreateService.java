...
@Service
public class SponsorNonComercalBannerCreateService implements AbstractCreateService<Sponsor, NonComercialBanner> {
	...
	public void create(final Request<NonComercialBanner> request, final NonComercialBanner entity) {
		assert request != null;
		entity.setSponsor(this.repository.findOneSponsorById(request.getPrincipal().getActiveRoleId()));
